...
def conv2d_valid_act(input_, output_dim, k_h=3, k_w=3, d_h=1, d_w=1, bias=0.0, with_logit=False, name="conv2d"):
    with tf.variable_scope(name):
...
def conv2d_valid_repeat(input_, output_dim, k_h=3, k_w=3, d_h=1, d_w=1, num_repeat=3, activation_fn=default_act_fn, name="conv_block"):
    with tf.variable_scope(name):
        for i in range(num_repeat):
            name = "conv_%d" % i
            output = conv2d_valid_act(input_, output_dim, k_h, k_w, d_h, d_w, activation_fn=activation_fn, name=name)
            input_ = output
        return output
...
def conv2d_same_act(input_, output_dim, k_h=3, k_w=3, d_h=1, d_w=1, bias=0.0, activation_fn=default_act_fn, with_logit=False, name="conv2d"):
    with tf.variable_scope(name):
        w = tf.get_variable('w', [k_h, k_w, input_.get_shape()[-1], output_dim], initializer=tf.contrib.layers.xavier_initializer())
        conv = tf.nn.conv2d(input_, w, strides=[1, d_h, d_w, 1], padding='SAME', name="conv")
        biases = tf.get_variable('b', [output_dim], initializer=tf.constant_initializer(bias))
...
def conv2d_same_repeat(input_, output_dim, k_h=3, k_w=3, d_h=1, d_w=1, num_repeat=3, activation_fn=default_act_fn, name="conv_block"):
    with tf.variable_scope(name):
        for i in range(num_repeat):
            name = "conv_%d" % i
            output = conv2d_same_act(input_, output_dim, k_h, k_w, d_h, d_w, activation_fn=activation_fn, name=name)
            input_ = output
        return output
...
def conv2dTrans_valid_act(input_, output_shape, k_h=3, k_w=3, d_h=1, d_w=1, bias=0.0, activation_fn=default_act_fn, with_logit=False, name="conv2dTrans"):
    with tf.variable_scope(name):
        w = tf.get_variable('w', [k_h, k_w, output_shape[-1], input_.get_shape()[-1]], initializer=tf.contrib.layers.xavier_initializer())
        conv = tf.nn.conv2d_transpose(input_, w, output_shape=output_shape, strides=[1, d_h, d_w, 1], padding="VALID")
        biases = tf.get_variable('biases', [output_shape[-1]], initializer=tf.constant_initializer(bias))
...
def conv2dTrans_same_act(input_, output_shape, k_h=5, k_w=5, d_h=2, d_w=2, bias=0.0, activation_fn=default_act_fn, with_logit=False, name="conv2dTrans"):
    with tf.variable_scope(name):
        w = tf.get_variable('w', [k_h, k_w, output_shape[-1], input_.get_shape()[-1]], initializer=tf.contrib.layers.xavier_initializer())
        conv = tf.nn.conv2d_transpose(input_, w, output_shape=output_shape, strides=[1, d_h, d_w, 1], padding="SAME")
        biases = tf.get_variable('biases', [output_shape[-1]], initializer=tf.constant_initializer(bias))
...
def bottleneck_layer(input_, output_dim, name="bottleneck"):
    return conv2d_same(input_, output_dim, k_h=1, k_w=1, d_h=1, d_w=1, bias=0.0, name=name)


def linear(input_, output_size, stddev=0.02, bias=0.0, name="linear"):
    input_size = input_.get_shape().as_list()[1]
    with tf.variable_scope(name):
        w = tf.get_variable('w', [input_size, output_size], tf.float32, initializer=tf.random_normal_initializer(stddev=stddev))
